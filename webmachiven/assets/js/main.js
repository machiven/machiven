const translations = {
  en: {
    ...
    formCaptcha: "Please verify you're human *",
    ...
    captchaRequired: 'Please complete the reCAPTCHA before submitting.'
  },
  es: {
    ...
    heroCopy: 'Suministramos maquinaria en stock, encontramos equipos difíciles de conseguir y ofrecemos servicios expertos de reparación para operaciones industriales.',
    ctaQuoteMain: 'Solicitar cotización',
    ...
    service1Link: 'Ver tienda →',
    service2Title: 'Busca y consigue equipos',
    service2Copy: '¿Necesitas una máquina o pieza específica? Conseguimos equipos difíciles de encontrar con reportes de inspección completos.',
    service2Link: 'Solicitar búsqueda →',
    service3Title: 'Reparación y mantenimiento',
    service3Copy: 'Servicios de reparación en sitio y remotos, búsqueda de piezas y mantenimiento preventivo para tu flota.',
    service3Link: 'Saber más →',
    ...
    product1Desc: 'Centro de mecanizado de precisión con cambiador de herramientas, totalmente inspeccionado y nivelado.',
    product2Desc: 'Prensa plegadora de alto tonelaje con tope trasero programable y sistemas de seguridad.',
    product3Desc: 'Montacargas pesado con certificación de carga y capacitación de operadores incluida.',
    productQuote: 'Solicitar cotización',
    repairsTitle: 'Servicios de reparación y mantenimiento',
    repairsCopy: 'Mantén tu equipo funcionando con soporte técnico experto.',
    repair1: 'Reparaciones y diagnósticos en sitio',
    repair1Desc: 'Técnicos se desplazan a tu instalación para averías urgentes y mantenimiento programado.',
    repair2: 'Búsqueda y reemplazo de piezas',
    repair2Desc: 'Mantenemos consumibles críticos y conseguimos piezas OEM/aftermarket para entrega rápida.',
    ...
    repairCta: 'Solicitar cotización de reparación',
    aboutEyebrow: 'Acerca de Machiven LLC',
    aboutTitle: 'Ingenieros de campo que entienden el uptime.',
    aboutCopy: 'Combinamos procura, inspección técnica y logística para que recibas maquinaria que funciona. Desde la primera llamada hasta la instalación y más allá, trabajas directamente con ingenieros y técnicos.',
    metaYears: 'Años en la industria',
    ...
    adminDesc: 'Gestiona artículos de la tienda.',
    adminNoAccess: 'Zona restringida. Inicia sesión como admin.',
    ...
    productSlug: 'Slug',
    productOrder: 'Orden',
    addressLabel: 'Contáctanos',
    contactTitle: '¿Listo para comenzar?',
    contactCopy: 'Solicita una cotización para compra, búsqueda o reparaciones.',
    contactQuote: 'Solicitar cotización',
    contactChat: 'Chatea en WhatsApp',
    contactEmail: 'Envía correo a admin@machiven.com',
    ...
    formTypeRepair: 'Servicio de reparación',
    ...
    formCaptcha: 'Por favor verifica que no eres un robot *',
    formFirstNamePh: 'Juan',
    formLastNamePh: 'Pérez',
    formEmailPh: 'juan@empresa.com',
    formPhonePh: '+52...',
    formCompanyPh: 'Nombre de la empresa',
    formProductPh: 'Detalles del producto o pieza',
    formMessagePh: 'Cuéntanos sobre tus necesidades, especificaciones o plazos...',
    captchaRequired: 'Por favor completa el reCAPTCHA antes de enviar.'
  }
};
  ...
  if (imageFileInput && imageFileInput.files && imageFileInput.files[0]) {
    ...
    if (uploadError) {
      if (uploadError.message.includes('not found')) {
        alert(`El bucket "${SUPABASE_BUCKET}" no existe. Por favor créalo en Supabase Dashboard: Storage > New Bucket > Nombre: "${SUPABASE_BUCKET}" > Public: Yes`);
      } else {
...
if (quoteForm && modal) {
  ...
  quoteForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const captchaResponse = (window.grecaptcha && typeof window.grecaptcha.getResponse === 'function')
      ? window.grecaptcha.getResponse()
      : '';
    if (!captchaResponse) {
      alert(translations[currentLang].captchaRequired || 'Please complete the reCAPTCHA before submitting.');
      return;
    }
    ...
    try {
      const response = await fetch('https://sggwlqpahbozwxbgyvnz.supabase.co/functions/v1/send-quote-email', {
        ...
        body: JSON.stringify({ ...payload, captchaToken: captchaResponse })
      });
...
function renderPublicProducts(items) {
  ...
  items.forEach(item => {
    ...
    const priceText = item.price != null ? `$${Number(item.price).toLocaleString()}` : (currentLang === 'en' ? 'Request quote for pricing' : 'Solicitar cotización para precio');
